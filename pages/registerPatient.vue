<template>
    <v-container>
        <v-form ref="form" v-model="valid" lazy-validation>
            <v-row>
                <v-col cols="12" md="4">
                    <v-text-field id="fName" :rules="nameRules" label="First name" required></v-text-field>
                </v-col>

                <v-col cols="12" md="4">
                    <v-text-field id="lName" :rules="nameRules" label="Last name" required></v-text-field>
                </v-col>
            ...
            <v-text-field id="devId" :counter="10" :rules="deviceRules" label="Device ID" required></v-text-field>

            <v-text-field id="phone" :rules="phoneRules" label="Patient Phone Number" required></v-text-field>

            <v-text-field id="email" :rules="emailRules" label="Patient E-mail" required></v-text-field>
            ...
            try {
                ...
                await createUserWithEmailAndPassword(auth2, document.getElementById("email").value,
                    'testtesttest').then(function (firebaseUser) {
                        ...
                        await updateProfile(auth2.currentUser, { displayName: document.getElementById("lName").value + " " +document.getElementById("fName").value });
                        await this.$fire.database.ref(this.$fire.auth.currentUser.displayName + '/patients/' + auth2.currentUser.displayName).set({
                            ...
                            emergencyContacts: [document.getElementById("email").value, document.getElementById("emer1").value, document.getElementById("emer2").value],
